...
// Fungsi: Generate Game Pin acak 6 digit
...
export default function QuestionListPage() {
  ...
  // Parse favorites from authProfile on load and when authProfile changes
  useEffect(() => {
    if (authProfile) {
      try {
  ...
  // Generate category list from quizzes for category filter dropdown
  ...
  // Reset quizzes and page when user changes (e.g. login/logout)
  useEffect(() => {
  ...
  // Fetch quizzes with optimized logic based on current filters and pagination
  useEffect(() => {
    ...
    fetchQuizzes();
  }, [authProfile?.id, authLoading, currentPage, searchQuery, selectedCategory, favoritesMode, myQuizzesMode]);
  ...
  // Handler for selecting a quiz to host
  ...
  // Toggle favorite/unfavorite quiz
        ...
        <div className="w-full px-4 py-4 pb-0 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:block">
              <Image src="/crazyrace-logo.webp" alt="Crazy Race" width={270} height={50} sizes="270px" style={{ imageRendering: 'auto' }} className="h-auto drop-shadow-xl cursor-pointer" onClick={() => router.push('/')} />
            </div>
          </div>
        ...
        <div className="relative container mx-auto px-6 py-8 pt-0 max-w-6xl">
          ...
          <div className="text-center sm:m-7">
            <motion.div
              ...
              className="pixel-border-large inline-block p-2 pt-0"
            >
              <h1 className="text-4xl md:text-5xl font-bold text-[#ffefff] pixel-text glow-pink">
                {t('soal.title')}
              </h1>
            </motion.div>
          </div>
